...
from hgcn.graph.ntu_rgb_d import Graph
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3, hyper_joints=0,
        ...
        if graph is None:
            ...
            self.graph = Graph(hyper_joints=3, labeling_mode='virtual_ensemble')
    ...
    def forward(self, x):
        ...
        x = self.drop_out(x)
        op = self.fc(x)
        return op, x, [h_x1, h_x2, h_x3, h_x4, h_x5, h_x6, h_x7, h_x8, h_x9]

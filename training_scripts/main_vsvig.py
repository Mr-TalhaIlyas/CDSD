...
from feeders.feeder_vsvig import Feeder, custom_collate_fn, LABEL_NAMES
...
class Trainer:
    def __init__(self, config_path):
        ...
        print(30*'=~')
        # chkpt = '/mnt/ssd/Talha/reason/work_dir/ntu_120_xsub_joint_96c2/best_model.pth'
        chkpt = '/mnt/ssd/Talha/reason/work_dir/ntu_120_xsub_joint_96c2/checkpoint_epoch_12.pth'
        ckpt = torch.load(chkpt, map_location='cpu')
        state_dict = ckpt['model_state']
        # print how many chkepoints got loaded successfully
        
        model_state = self.model.state_dict()
        filtered_state = {}
        n_loaded = 0
        print('total params in model: {}, total params in checkpoint: {}'.format(len(model_state), len(state_dict)))
        for k, v in state_dict.items():
            if k in model_state and v.shape == model_state[k].shape:
                ...
                # print(f"Skipping {k}: {v.shape}")
                pass
        print(f"✅ Loaded {n_loaded}/{len(model_state)} parameters from checkpoint.")
        ...
        print("\nUnexpected keys (ignored from checkpoint):")
        print(len(unexpected_keys))
        # for k in unexpected_keys:
        #     print(k)
        
        self.model.to(self.device)
        print('Model loaded from {}'.format(chkpt))
        # freeze skeleton_model from  model, print num of layers frozen and trainable
        
        # n_frozen = 0
        # n_trainable = 0
        # for name, param in self.model.named_parameters():
        #     if 'skeleton_model' in name:
        #         param.requires_grad = False
        #         n_frozen += 1
        #     else:
        #         n_trainable += 1
        # print(f"Number of frozen layers: {n_frozen}")
        # print(f"Number of trainable layers: {n_trainable}")
        print(30*'=~')
...
def main():
    cfg = os.path.join(os.path.dirname(__file__), 'config', 'szr', 'config_skeleton_vsvig.yaml')

...
from model.model_supervised import SkeletonACL_CLIP_Logic
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        for step, (batch_data, batch_label, _, _) in pbar:
            ...
            # Forward pass
            y_pred, loss = self.model(batch_data, batch_label)
        ...
        print(f"\n{'='*60}")
        print(f"EPOCH {epoch} TRAIN: Loss={epoch_loss:.4f}, Acc={acc:.2f}%, Top5={top5:.2f}%")
        print(f"{'='*60}")
    ...
    def evaluate(self, epoch, split='test'):
        ...
        for batch_data, batch_label, _, _ in pbar:
            batch_data = batch_data.to(self.device)
            batch_label = batch_label.to(self.device)
            ...
            y_pred = self.model(batch_data, batch_label)
        ...
        print(f"EPOCH {epoch} EVAL: Acc={acc:.2f}%, Top5={top5:.2f}%")

...
from classes_pallets import COCO_WHOLEBODY_SKELETON_INFO, COCO_WHOLEBODY_KPTS_COLORS
...
COCO_WHOLEBODY_SKELETON = [
    info['link'] for idx, info in sorted(COCO_WHOLEBODY_SKELETON_INFO.items())
]
...
class SapiensPoseEstimator:
    ...
    def get_coco_keypoints(self, keypoints: np.ndarray) -> np.ndarray:
        ...
        if self.model_type == SapiensModelType.COCO_17:
            return keypoints
        elif self.model_type == SapiensModelType.COCO_133:
            return keypoints
        else:
            return self.get_goliath_keypoints(keypoints)
    ...
    def draw_pose_wholebody(
        self,
        image: np.ndarray,
        keypoints: np.ndarray,
        conf_threshold: float = 0.3,
        skeleton_color: Tuple[int, int, int] = (0, 255, 0),
        keypoint_color: List[Tuple[int, int, int]] = COCO_WHOLEBODY_KPTS_COLORS,
        thickness: int = 2,
        radius: int = 4,
        draw_skeleton: bool = True,
    ) -> np.ndarray:
        ...
        Args:
            ...
            conf_threshold: Min confidence to draw
            skeleton_color: BGR color for skeleton
            keypoint_color: List of BGR colors for keypoints
            thickness: Line thickness
            radius: Keypoint radius
            draw_skeleton: Whether to draw skeleton connections
        ...
        # Only draw skeleton for COCO wholebody-compatible keypoints
        if draw_skeleton and len(keypoints) >= 133:
            for i, j in COCO_WHOLEBODY_SKELETON:
                if (i < len(keypoints) and j < len(keypoints) and
                    keypoints[i, 2] > conf_threshold and
                    keypoints[j, 2] > conf_threshold):
        ...
        for i, (x, y, conf) in enumerate(keypoints):
            if conf > conf_threshold:
                cv2.circle(result, (int(x), int(y)), radius, keypoint_color[i], -1)
                cv2.circle(result, (int(x), int(y)), radius, (255, 255, 255), 1)
...
if __name__ == '__main__':
    ...
    model_path = '/data_hdd/talha/miccai_26/seizure_detection_pipeline/weights/sapiens_2b_coco_wholebody_best_coco_wholebody_AP_745_torchscript.pt2'
    image_path = '/data_hdd/talha/miccai_26/seizure_detection_pipeline/tracker/viz.png'
    ...
    # Draw and save
    if estimator.model_type == SapiensModelType.COCO_133:
        result = estimator.draw_pose_wholebody(image, keypoints,thickness = 6,
        radius= 8)
    else:

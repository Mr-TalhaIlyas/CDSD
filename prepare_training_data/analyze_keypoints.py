...
# ntu_kps = convert_coco_to_ntu(keypoints)
# print(f"Converted to NTU keypoints shape: {ntu_kps.shape}")

# # Visualize some frames with NTU keypoints
# for frame_idx in range(0, ntu_kps.shape[0]):
#     frame_kps = ntu_kps[frame_idx]
#     vis_image = draw_pose(image, frame_kps,
#                           skeleton_connections=NTU_SKELETON_CONNECTIONS)
#     clear_output(wait=True)
#     plt.imshow(cv2.cvtColor(vis_image, cv2.COLOR_BGR2RGB))
#     plt.title(f'Frame {frame_idx} (NTU)')
#     plt.axis('off')
#     plt.show()
#     plt.pause(0.05)
#%%

# ntu_kps = convert_coco_to_ntu(keypoints, True)
# ntu_kps, _ = remove_zero_frames(ntu_kps, None)
# ntu_kps = normalize_skeleton_to_ntu_scale(ntu_kps)
# print(f"Converted to NTU keypoints shape: {ntu_kps.shape}")
...
processed_file = Path('D:/miccai_26/DATA/processed/vsvig_seizure_dataset.npz')
...
# radnomply plot 100 samples
# %%
import random
processed_file = Path('D:/miccai_26/DATA/processed/ieee_seizure_dataset.npz')
if processed_file.exists():
    processed_data = np.load(processed_file, allow_pickle=True)
    ...
    sample_indices = random.sample(range(processed_data['data'].shape[0]), 100)
